...
Initial internal release of the specification draft.

### Added

- The `did:dplaax` DID method and registry model.
- The DID grant-type namespace document under `site/` for
  `https://dplaax.dev/oauth/grant-type/did` (published at the wire-identifier
  path when the spec goes public; the identifier is compared byte-for-byte).
